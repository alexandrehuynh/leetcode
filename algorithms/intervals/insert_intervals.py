...
class Solution:
    def insert(self, intervals: List[List[int]], newInterval: List[int]) -> List[List[int]]:
        ...
        i = 0  # Pointer for traversing the intervals
        n = len(intervals)

        # Step 1: Add all intervals that end before newInterval starts
        while i < n and intervals[i][1] < newInterval[0]:
            result.append(intervals[i])  # No overlap, add directly to result
            i += 1

        # Step 2: Merge overlapping intervals with newInterval
        while i < n and intervals[i][0] <= newInterval[1]:
            # Update the newInterval to include the overlapping interval
        ...
        return result  # Return the final merged list
